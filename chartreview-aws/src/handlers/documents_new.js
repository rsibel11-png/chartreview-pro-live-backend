...
Return ONLY a JSON object with these exact fields:
{
  ...
  "low_relevance_pages": [{"page_number": 1, "reason": "string"}]
    ...
    // ── VI pre-pass: build encounter_index for this doc part ─────────────────
    // Runs after Textract so extracted_text is available for fallback reference.
    // Uses the same S3 PDF bytes already fetched by assessRelevance.
    // Result is stored as encounter_index on the DynamoDB record so the summary
    // coordinator can read it without a redundant Bedrock call at generation time.
    // Non-fatal: if it fails, coordinator falls back to full-document extraction.
    try {
      // Only run VI pre-pass on non-rejected clinical docs with real text
      const isRejectedDoc = doc.is_rejected === true;
      const hasText = extractedText && extractedText.length > 200 && !extractedText.startsWith('[Textract');
      if (!isRejectedDoc && hasText) {
        console.log('viPrePass: starting for', aws_document_id);

        // Fetch PDF from S3 (same as assessRelevance — re-fetch since buffer not in scope)
        const viS3Object = await s3.send(new GetObjectCommand({ Bucket: BUCKET, Key: doc.file_key }));
        const viPdfChunks = [];
        for await (const chunk of viS3Object.Body) { viPdfChunks.push(chunk); }
        const viPdfBase64 = Buffer.concat(viPdfChunks).toString('base64');

        const viPrompt = `You are a medical record analyst. Review this PDF document and identify every distinct clinical encounter (office visit, ER visit, consultation, surgery, radiology, etc.).

For EACH encounter found, return:
- date: exact service date in YYYY-MM-DD format. Use SERVICE DT, REP SRV DT, or TRIAGE DATE if present — NOT the admission date (ADM DT) or electronic signature date. IGNORE time values (e.g. "1825") entirely — they are not dates.
- provider: full name with credentials (e.g. "Tall, Samantha M, MD")
- facility: facility name
- visit_type: one of "Office Visit", "ER Visit", "Hospitalist Progress Note", "History & Physical", "Consultation Report", "Operative Report", "Radiology", "C-4 Form", "Discharge Summary", "Other"
- pages: array of 1-based page numbers belonging to this encounter

Exclude: lab-only printouts, appointment reminders, face sheets, consent forms, coding summaries, PACU/anesthesia-only entries, medication order lists.

Return ONLY valid JSON: { "visits": [ { "date": "YYYY-MM-DD", "provider": "...", "facility": "...", "visit_type": "...", "pages": [1,2,3] } ] }`;
        ...
        const viPayload = {
          ...
          messages: [{
            role: 'user',
            content: [
              { type: 'document', source: { type: 'base64', media_type: 'application/pdf', data: viPdfBase64 } },
              { type: 'text', text: viPrompt },
            ],
          }],
        };

        const viResp = await bedrock.send(new InvokeModelCommand({
          modelId: 'us.anthropic.claude-sonnet-4-6',
          contentType: 'application/json',
          accept: 'application/json',
        ...
        const viBody = JSON.parse(new TextDecoder().decode(viResp.body));
        const viRaw = viBody.content?.[0]?.text || '';

        let viResult = null;
        try {
          const viMatch = viRaw.match(/\{[\s\S]*\}/);
          viResult = viMatch ? JSON.parse(viMatch[0]) : null;
        } catch (e) {
          console.warn('viPrePass: JSON parse error', e.message);
        }

        if (viResult && Array.isArray(viResult.visits)) {
          const normalizeViDate = (raw) => {
            const d = (raw || '').trim().replace(/[T\s]\d{1,2}:\d{2}.*$/, '').trim();
            if (/^\d{4}-\d{2}-\d{2}$/.test(d)) return d;
            const m = d.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})$/);
            if (m) { const yr = m[3].length === 2 ? '20' + m[3] : m[3]; return `${yr}-${m[1].padStart(2,'0')}-${m[2].padStart(2,'0')}`; }
            return '';
          };
          const encounterIndex = viResult.visits
            .map(v => ({
              date:       normalizeViDate(v.date),
              provider:   v.provider || '',
              facility:   v.facility || '',
          ...
          console.log('viPrePass: saved', encounterIndex.length, 'visits to encounter_index for', aws_document_id);
        } else {
          console.warn('viPrePass: no parseable visits for', aws_document_id);
        }
      }
    } catch (viErr) {
      console.warn('viPrePass: failed (non-fatal):', viErr.message);
    }
    // ── end VI pre-pass ───────────────────────────────────────────────────────
