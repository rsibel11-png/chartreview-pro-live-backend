...
E) C-4 FORMS (Workers' Compensation Board Doctor's Report / WCB Form C-4):
    ...
    - SINGLE ENCOUNTER RULE (2026-09-21): A C-4 form is exactly ONE clinical encounter. Extract it as ONE visit entry. Do NOT create additional, separate visit entries just because the C-4 page has multiple distinct fields (injury narrative, diagnosis, treatment/tests ordered, X-ray/imaging findings notation, etc.) — those are all part of the SAME single C-4 encounter, not separate visits.
    - DO NOT FABRICATE A RADIOLOGY/TEST VISIT FROM THE C-4 FORM'S OWN NOTATION (2026-09-26): Some C-4 forms print a brief inline X-ray/imaging notation directly on the claim form itself (e.g., "X-Ray Findings: No fractures"). That notation is part of the SAME single C-4 encounter — never extract it as its own "Radiology" or "imaging" visit, whether the C-4's own notation says a test was merely ordered, gives a one-line result, or describes findings in more detail. Only extract a separate radiology visit if a genuinely distinct radiology report document exists elsewhere in the document set — its own letterhead, its own exam date/time, its own signing radiologist's signature block, on its own page(s). Never borrow a radiologist's name from a different report to attach to the C-4's inline notation.
